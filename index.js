  ...
  return new Promise(function (resolve, reject) {
    ...
    var request = http.get(url, function (response) {
      ...
      response.on('end', function () {
        ...
        console.log($("#ctl00_ContentPlaceHolderMainContent_ReviewSummary").text());
        console.log($("#ctl00_ContentPlaceHolderMainContent_RatingText").text());
        resolve('success');
      });
    });
...
async function getReview(callback) {
  var data = '';
  http.get('http://www.nowrunning.com/cgi-bin/rss/reviews_malayalam.xml', async function (res) {
    if (res.statusCode >= 200 && res.statusCode < 400) {
      ...
      res.on('end', async function () {
        parser.parseString(data, async function (err, result) {
          for (let i = 0; i < 4; i++) {
            callb(result['rss']['channel'][0]['item'][i])
          }
        });
      });
    }
   ...
   const cardTitle = 'hello';
    const speechOutput = 'kooi';
    // Setting this to true ends the session and exits the skill.
    const shouldEndSession = false;
    callback({}, buildSpeechletResponse(cardTitle, speechOutput, null, false));
};

async function callb(element) {
  console.log(element.title[0]);
